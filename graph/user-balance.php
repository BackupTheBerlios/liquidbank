...
$user_graph =  "../cache/user-web-{$user->name}.png";
...
if ( $update ){
	...
	if ( $result ) {
		...
		fwrite($fd, "}\n");
		fclose($fd);
		system("fdp -Tcmapx -o$tmpfname.map -Tpng -o$tmpfname.png $tmpfname ");

		unlink("$tmpfname");
		rename("$tmpfname.png", $user_graph );
		rename("$tmpfname.map", $user_map );
	}
